...
impl SignatureContext<'_> {
    ...
    pub async fn v4_check_presigned_url(&mut self) -> S3Result<CredentialsExt> {
        ...
        let raw_signature = if signature == expected_signature || !has_unencoded_reserved_path_char(self.raw_uri_path) {
            None
        } else {
            let canonical_request = sig_v4::create_presigned_canonical_request_with_raw_uri_path(
                method,
                self.raw_uri_path,
                qs.as_ref(),
                &headers,
